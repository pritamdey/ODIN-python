...
from . import mat_vec_transforms
...
class ODIN:
    ...
    def __logistic_predict(self):
        return math_functions.expit(self.z + np.matmul(self.x, self.beta))
    ...
    def __fit_model(self, print_out, init_z, init_beta):
        ...
        while np.absolute(diff) > self.tol and iterate < self.maxiter:
            ...
            # Norm of gradient
            # norms[iterate - 1] = m.l2_norm(np.array([m.l2_norm(direction_z), m.l2_norm(direction_beta)]))
    ...
    def __calculate_influence(self, print_out):
        ...
        for i in range(n):
            ...
            self.im2[i] = math_functions.quadratic_form(beta_i_diff, s_inv)

        t = np.linalg.inv(t)
        z_influence = (n / (n - 1)) * np.matmul(t, z_influence)

        for i in range(n):
            self.im1[i] = math_functions.l2_norm(z_influence[:, i])
    ...
    def fit_and_detect_outliers(self, a, lobes=None, hemis=None, x=None, init_z=None, init_beta=None, print_out=False):
        start = time()
        if len(a.shape) == 3:
            self.a = mat_vec_transforms.half_vectorize_all(a)
        else:
            self.a = a
        if x is None:
            self.x = make_x.make_x(lobes, hemis)
        else:
            self.x = x
